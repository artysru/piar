...
$result = mysql_query("SELECT Message163.Message_ID, Message163.Sub_Class_ID, Message163.Article, Message163.Name, Message163.height, Message163.Width, Message163.Free, Message163.Price, Sub_Class.EnglishName FROM Message163 JOIN Sub_Class on Message163.Sub_Class_ID = Sub_Class.Sub_Class_ID and Parrent = ".$f_Article." and Active = 1 and ".$value_cheak_free." ".$value_cheak_busy." WHERE ".$value_cheak_depend." ORDER BY Article");
while ($row = mysql_fetch_array($result)){
echo '<tr><td>'.$f_Article.'.'.$row["Article"].'</td><td><a href="/'.$current_sub['EnglishName'].'/'.$row["EnglishName"].'/'.$row["EnglishName"].'_'.$row["Message_ID"].'.html">'.$row["Name"].'</a></td><td>'.$row["Width"].' x '.$row["height"].'</td><td>'.$row["Price"].'</td><td><img width="90" src='.nc_file_path(163, $row["Message_ID"], "image_1", "h_").' /></td><td>'.$row["Free"].'</td></tr>';
}
echo "</table>";
$result .="